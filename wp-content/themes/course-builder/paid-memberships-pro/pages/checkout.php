...
<div id="pmpro_level-<?php echo $pmpro_level->id; ?>">
	<form id="pmpro_form" class="pmpro_form" action="<?php if(!empty($_REQUEST['review'])) echo pmpro_url("checkout", "?level=" . $pmpro_level->id); ?>" method="post">
		...
		<div id="pmpro_pricing_fields" class="pmpro_checkout">
			<h3>
				<span class="pmpro_checkout-h3-name"><?php esc_html_e('Membership Level', 'course-builder' );?></span>
				<?php if(count($pmpro_levels) > 1) { ?><span class="pmpro_checkout-h3-msg"><a href="<?php echo pmpro_url("levels"); ?>"><?php esc_html_e('change', 'course-builder' );?></a></span><?php } ?>
			</h3>
			<div class="pmpro_checkout-fields">
				...
				<?php if($pmpro_show_discount_code) { ?>
					<?php if($discount_code && !$pmpro_review) { ?>
						<p id="other_discount_code_p" class="pmpro_small"><a id="other_discount_code_a" href="#discount_code"><?php esc_html_e('Click here to change your discount code', 'course-builder' );?></a>.</p>
					<?php } elseif(!$pmpro_review) { ?>
						<p id="other_discount_code_p" class="pmpro_small"><?php esc_html_e('Do you have a discount code?', 'course-builder' );?> <a id="other_discount_code_a" href="#discount_code"><?php esc_html_e('Click here to enter your discount code', 'course-builder' );?></a>.</p>
					<?php } elseif($pmpro_review && $discount_code) { ?>
						<p><strong><?php esc_html_e('Discount Code', 'course-builder' );?>:</strong> <?php echo $discount_code?></p>
					<?php } ?>
				<?php } ?>
		...
		<?php if(!$skip_account_fields && !$pmpro_review) { ?>
			...
			<div id="pmpro_user_fields" class="pmpro_checkout">
				<h3>
					<span class="pmpro_checkout-h3-name"><?php esc_html_e('Account Information', 'course-builder' );?></span>
					<span class="pmpro_checkout-h3-msg"><?php esc_html_e('Already have an account?', 'course-builder' );?> <a href="<?php echo add_query_arg( 'redirect_to', pmpro_url( "checkout", "?level=" . $pmpro_level->id ), thim_get_login_page_url() ); ?>"><?php esc_html_e('Log in here', 'course-builder' );?></a></span>
				</h3>
				<div class="pmpro_checkout-fields">
					...
					<div class="pmpro_hidden">
						...
						<input id="fullname" name="fullname" type="text" class="input <?php echo pmpro_getClassForField("fullname");?>" size="30" value="" /> <strong><?php esc_html_e('LEAVE THIS BLANK', 'course-builder' );?></strong>
		...
		<?php if(pmpro_getGateway() == "paypal" && empty($pmpro_review) && true == apply_filters('pmpro_include_payment_option_for_paypal', true ) ) { ?>
			<div id="pmpro_payment_method" class="pmpro_checkout" <?php if(!$pmpro_requirebilling) { ?>style="display: none;"<?php } ?>>
				<hr />
				<h3>
					<span class="pmpro_checkout-h3-name"><?php esc_html_e('Choose your Payment Method', 'course-builder' ); ?></span>
				</h3>
				<div class="pmpro_checkout-fields">
		...
		if($pmpro_include_billing_address_fields) { ?>
			<div id="pmpro_billing_address_fields" class="pmpro_checkout" <?php if(!$pmpro_requirebilling || apply_filters("pmpro_hide_billing_address_fields", false) ){ ?>style="display: none;"<?php } ?>>
				<hr />
				<h3>
					<span class="pmpro_checkout-h3-name"><?php esc_html_e('Billing Address', 'course-builder' );?></span>
				</h3>
				<div class="pmpro_checkout-fields">
		...
		if($pmpro_include_payment_information_fields) { ?>
			<div id="pmpro_payment_information_fields" class="pmpro_checkout" <?php if(!$pmpro_requirebilling || apply_filters("pmpro_hide_payment_information_fields", false) ) { ?>style="display: none;"<?php } ?>>
				<hr />
				<h3>
					<span class="pmpro_checkout-h3-name"><?php esc_html_e('Payment Information', 'course-builder' );?></span>
					<span class="pmpro_checkout-h3-msg"><?php printf(__('We Accept %s', 'course-builder' ), $pmpro_accepted_credit_cards_string);?></span>
				</h3>
		...
		<?php if($tospage && !$pmpro_review) { ?>
			<div id="pmpro_tos_fields" class="pmpro_checkout">
				...
				<div class="pmpro_checkout-fields">
					...
					<input type="checkbox" name="tos" value="1" id="tos" /> <label class="pmpro_label-inline pmpro_clickable" for="tos"><?php printf(__('I agree to the %s', 'course-builder' ), $tospage->post_title);?></label>
				</div> <!-- end pmpro_checkout-fields -->
		...
		<div class="pmpro_submit">
			...
			<?php } else { ?>
				...
				if($pmpro_checkout_default_submit_button)
				{
					...
					<input type="submit" class="pmpro_btn pmpro_btn-submit-checkout" value="<?php if($pmpro_requirebilling) { esc_html_e('Submit and Check Out', 'course-builder' ); } else { esc_html_e('Submit and Confirm', 'course-builder' );}?> &raquo;" />
				</span>
					<?php
				}
				?>
